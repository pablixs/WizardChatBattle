...
export const ScreenEffects: React.FC<ScreenEffectsProps> = ({
  ...
  useEffect(() => {
    ...
    setLastHp(selfPlayer.hp);
  }, [selfPlayer.hp, lastHp]);

  // Detect incoming spell element in the latest log entry
  useEffect(() => {
    if (logs.length === 0) return;
    const latest = logs[0];
    // If the latest log entry affected self player as target
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40 overflow-hidden">
      {/* 1. Ice / Frozen Overlay */}
      {isFrozen && (
        <div className="absolute inset-0 bg-cyan-500/20 backdrop-blur-[2px] border-[16px] border-cyan-400/40 shadow-[inset_0_0_100px_rgba(6,182,212,0.8)] flex items-center justify-center animate-pulse">
          <div className="flex flex-col items-center gap-2 bg-slate-950/80 p-4 rounded-2xl border border-cyan-400/60 text-cyan-200">
            <Snowflake className="w-12 h-12 text-cyan-300 animate-spin" style={{ animationDuration: '3s' }} />
            <span className="font-extrabold text-sm uppercase tracking-widest">¡ESTÁS CONGELADO!</span>
          </div>
        </div>
      )}
